...
from op.models import OpLocation, OpLocationType, OpProfession, OpResources, OpPolitician, OpResourcesType, OpEducationLevel, OpInstitutionCharge, OpPoliticalCharge, OpOrganizationCharge


class LocationTypeResource(ModelResource):
    class Meta:
        queryset = OpLocationType.objects.using('op').all()
...
class LocationResource(ModelResource):
    ...
    class Meta:
        queryset = OpLocation.objects.using('op').all()
...
class EducationLevelResource(ModelResource):
    def get_resource_uri(self, bundle_or_obj):
        ...
        if bundle_or_obj.__class__.__name__ == 'OpPoliticianHasOpEducationLevel':
            return '/op/v1/%s/%s/' % (self._meta.resource_name,bundle_or_obj.obj.education_level.pk)
        else:
            return '/op/v1/%s/%s/' % (self._meta.resource_name,bundle_or_obj.obj.pk)
    class Meta:
        queryset = OpEducationLevel.objects.using('op').all()
        resource_name = 'education_level'
        allowed_methods = ['get',]

class ProfessionResource(ModelResource):
    class Meta:
        queryset = OpProfession.objects.using('op').all()
...
class ResourceTypeResource(ModelResource):
    class Meta:
        queryset = OpResourcesType.objects.using('op').all()
...
class ResourceResource(ModelResource):
    ...
    def get_resource_uri(self, bundle_or_obj):
        return '/op/v1/%s/%s/' % (self._meta.resource_name,bundle_or_obj.obj.content.pk)

    class Meta:
        queryset = OpResources.objects.using('op').all()
...
class ChargeResource(ModelResource):
    politician = fields.ForeignKey('op.v1.api.PoliticianResource', 'politician', full=True)
...
class InstitutionChargeResource(ChargeResource):
    ...
    class Meta(ChargeResource.Meta):
        queryset = OpInstitutionCharge.objects.using('op').all()
...
class PoliticalChargeResource(ChargeResource):
    location = fields.ForeignKey(LocationResource, 'location', null=True)
    class Meta(ChargeResource.Meta):
        queryset = OpPoliticalCharge.objects.using('op').all()
        resource_name = 'politcical_charge'

class OrganizationChargeResource(ChargeResource):
    class Meta(ChargeResource.Meta):
        queryset = OpOrganizationCharge.objects.using('op').all()
...
class PoliticianResource(ModelResource):
    ...
    class Meta:
        queryset = OpPolitician.objects.using('op').all()
